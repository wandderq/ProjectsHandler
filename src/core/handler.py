...
class ProjectsHandler:
    ...
    def init_project_files(self, project: Project) -> None:
        ...
        if all([file in spec_files for file in self.required_project_files.keys()]):
            overwrite = input(f'Project alreday exists. Overwrite it? y/n: ').lower().strip() == 'y'
            if not overwrite: return
        else:
            overwrite = input('Some project files alreday exist. Overwrite it? y/n: ').lower().strip() == 'y'
        ...
        for file_name, file_content in self.required_project_files.items():
            if (not file_name in spec_files) or overwrite:
                ...
                with open(file_path, 'w', encoding='utf-8') as file:
                    file.write(file_content.safe_substitute(
                        pname=project.name,
                        pjson=json.dumps(project.json(), indent=4, ensure_ascii=False)
                    ))
...
def parse_args() -> Namespace:
    argparser = ArgumentParser(
        description="Handler, Helper, Manager for all your's awesome projects"
    )
    custom_subparsers = argparser.add_subparsers(
        dest="command", help="Handle commands", required=True
    )

    init_parser = custom_subparsers.add_parser("init", help="Initialize new project")
    init_parser.add_argument(
        "-N", "--name", default="New Project", type=str, help="New project name"
    )
    init_parser.add_argument(
        "-V", "--version", default="0.1.0", type=str, help="New project version"
    )
    init_parser.add_argument(
        "-L",
        "--languages",
        default="Python3,C++",
        type=str,
        help="New project languages",
    )
    init_parser.add_argument(
        "-F",
        "--frameworks",
        default="flask,tkinter",
        type=str,
        help="New project frameworks",
    )
    init_parser.add_argument(
        "-D",
        "--description",
...
def main() -> int | None:
    ...
    args = parse_args()
    logger = setup_logger(
        name="projectshandler", level=(DEBUG if args.verbose else INFO)
    )
